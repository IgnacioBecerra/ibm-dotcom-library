...
class DDSFilterGroupItem extends StableSelectorMixin(BXAccordionItem) {
  ...
  protected _renderViewAll(): HTMLButtonElement {
    ...
    viewAll.classList.add(viewAllClassName, `${prefix}--input-container__heading`);
    viewAll.type = 'button';
    ...
    viewAll.addEventListener('click', (e): void => {
      ...
      [...children].slice(filterCutoff, children.length).forEach(elem => {
        (elem as HTMLElement).style.display = this.expanded ? '' : 'none';
      })
  
      this.dispatchEvent(
        new CustomEvent('viewAllRemoved', {
          bubbles: true,
          cancelable: true,
          composed: true,
          detail: {
  ...
  firstUpdated() {
    ...
    const viewRemoved = (((this.closest('dds-filter-panel')?.parentNode as ShadowRoot).host as HTMLElement) as any)._viewAllRemoved;


    console.log('coming from composite is: ', viewRemoved)
    ...
    if(!viewRemoved) {
      ...
      // but make sure to unhide the hidden items
      [...this.children].slice(filterCutoff, children.length).forEach(elem => {
        (elem as HTMLElement).style.display = this.expanded ? '' : 'none';
      })
    }
  }
